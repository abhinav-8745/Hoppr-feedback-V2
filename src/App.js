...
// ── FULL SCREEN OPTION GRID ────────────────────────────────────────────────────
// This is the key component — options fill the entire available screen height
function OptionGrid({ options, selected, onSelect, renderLabel }) {
        ...
        return (
          <button key={i} onClick={() => onSelect(opt)}
            style={{
              ...
              padding:"0 20px",
              borderRadius:14,
              border: sel ? "2px solid #39FF14" : "1.5px solid #1e1e1e",
              background: sel ? "#001800" : "#0f0f0f",
              boxShadow: sel ? "0 0 16px #39FF1433" : "none",
              cursor:"pointer",
            ...
            {sel && <span style={{color:"#39FF14",fontSize:22,flexShrink:0}}>✓</span>}
          </button>
        );
      })}
    </div>
...
function Stars({value, onChange, size=40}) {
  ...
  return (
    <div>
        ...
        {[1,2,3,4,5].map(s => {
          ...
          return <button key={s} onClick={()=>onChange?.(s)}
            ...
            style={{background:"none",border:"none",cursor:onChange?"pointer":"default",
              fontSize:size,lineHeight:1,padding:6,WebkitTapHighlightColor:"transparent",
      ...
      {(h??value)>0&&<div style={{textAlign:"center",color:"#39FF14",fontFamily:"'Space Mono',monospace",fontSize:13,marginTop:12,letterSpacing:1}}>{labels[h??value]}</div>}
    </div>
  );
}
        ...
        {[...Array(11).keys()].map(n => {
          ...
          return <button key={n} onClick={()=>onChange(n)}
            style={{width:46,height:46,borderRadius:10,
              border:sel?`2px solid ${col(n)}`:"2px solid #222",
...
function FeedbackPage({onAdmin}) {
  ...
  const reset = () => {
    ...
    setRating(0);setChips([]);setOther("");setMetrics({});setNps(null);
    setWantsCB(false);setContact("");setComment("");
  };
  ...
  // STYLES
  const HEADER_H = 110;
  const FOOTER_H = step===1 ? 130 : step>1 ? 110 : 80;
  const bodyH = vh - HEADER_H - FOOTER_H;
  ...
  const h1  = {fontSize:"clamp(20px,5vw,26px)",fontWeight:700,color:"#fff",marginBottom:14,fontFamily:"'Rajdhani',sans-serif",lineHeight:1.2};
  const CTAbtn = (active) => ({
    width:"100%",padding:"18px",borderRadius:14,border:"none",
    background:active?"#39FF14":"#1e1e1e",
  ...
  const backBtn = {width:"100%",padding:"16px",borderRadius:14,border:"1.5px solid #222",background:"transparent",color:"#555",fontWeight:700,fontSize:16,cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",WebkitTapHighlightColor:"transparent",marginTop:8};

  // Fullscreen layout for option-based steps
  ...
  if(step===10) return (
    <div style={{height:vh,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",textAlign:"center"}}>
      ...
      <button onClick={reset} style={{...CTAbtn(true),width:"auto",padding:"18px 40px",fontSize:16}}>Give More Feedback</button>
    </div>
  );
  ...
  return (
    <div style={{height:vh,background:"#000",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* HEADER */}
      <div style={{height:HEADER_H,padding:"16px 20px 0",flexShrink:0,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#333",letterSpacing:1,textAlign:"center"}}>STEP {actualStep} OF {TOTAL}</div>
      </div>

      {/* BODY */}
      {isFullscreen ? (
        // FULLSCREEN MODE — options fill entire middle area
        <div style={{flex:1,display:"flex",flexDirection:"column",padding:"12px 16px 0",overflow:"hidden"}}>
          <div style={{marginBottom:10}}>
            <span style={lbl}>
              {step===1?"Centre":step===2?"Division":step===3?"Role":"Sport"}
            </span>
            <div style={h1}>
              {step===1?"Which centre did you visit?":step===2?"What brings you here?":step===3?"And you are?":"Which sport did you play?"}
            </div>
          </div>

          {/* OPTION GRID — fills all remaining space */}
          {step===1&&(
            <OptionGrid
              options={CENTRES}
              selected={centre}
              onSelect={setCentre}
              renderLabel={opt=>`🏟️  ${opt}`}
            />
          )}
          {step===2&&(
            <div style={{display:"flex",flexDirection:"column",flex:1,gap:8}}>
              {[["🎓","Coaching","For Parents & Kids"],["🎾","Pay & Play","For walk-in players"]].map(([e,d,sub])=>(
                <button key={d} onClick={()=>setDiv(d)}
                  style={{
                    flex:1,width:"100%",display:"flex",flexDirection:"column",
                    alignItems:"flex-start",justifyContent:"center",
                    padding:"0 24px",borderRadius:14,
                    border:division===d?"2px solid #39FF14":"1.5px solid #1e1e1e",
                    background:division===d?"#001800":"#0f0f0f",
                    boxShadow:division===d?"0 0 16px #39FF1433":"none",
                    cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent",
                  }}>
                  <div style={{fontSize:36,marginBottom:8}}>{e}</div>
                  <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:"clamp(20px,5vw,26px)",color:division===d?"#39FF14":"#fff",marginBottom:4}}>{d}</div>
              ...
              {[["👨‍👩‍👧","Parent"],["🏃","Kid / Student"]].map(([e,r])=>(
                <button key={r} onClick={()=>setRole(r)}
                  style={{
                    ...
                    gap:20,padding:"0 24px",borderRadius:14,
                    border:role===r?"2px solid #39FF14":"1.5px solid #1e1e1e",
                    background:role===r?"#001800":"#0f0f0f",
                    boxShadow:role===r?"0 0 16px #39FF1433":"none",
                    cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent",
                  }}>
                  <span style={{fontSize:44}}>{e}</span>
                  <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:"clamp(20px,5vw,26px)",color:role===r?"#39FF14":"#ccc"}}>{r}</span>
                </button>
              ))}
            </div>
          )}
        ...
        // SCROLLABLE MODE — for rating, chips, metrics etc
        <div style={{flex:1,overflowY:"auto",padding:"16px 20px",WebkitOverflowScrolling:"touch"}}>
          {step===5&&<>
            <span style={lbl}>Overall Rating</span>
            <div style={h1}>How was your session?</div>
            <div style={{marginTop:40,marginBottom:32}}><Stars value={rating} onChange={setRating} size={52}/></div>
          </>}
          {step===6&&<>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:4}}>
              {chipOptions.map(c=>(
                <button key={c} onClick={()=>toggle(c)}
                  style={{padding:"13px 18px",borderRadius:22,cursor:"pointer",fontSize:14,fontFamily:"'Space Mono',monospace",WebkitTapHighlightColor:"transparent",
                    ...
                    boxShadow:chips.includes(c)?"0 0 8px #39FF1433":"none"}}>
                  {c}
                </button>
              ))}
            </div>
            {chips.includes("Something else")&&(
              <textarea value={otherText} onChange={e=>setOther(e.target.value)} placeholder="Tell us more..." rows={3} style={{...inp,marginTop:14}}/>
            )}
          </>}
          {step===7&&<>
            ...
            {metricsList.map(m=>(
              <div key={m} style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:14,padding:"18px",marginBottom:12}}>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:12,color:"#777",marginBottom:14}}>{m}</div>
                <Stars size={36} value={metrics[m]||0} onChange={v=>setMetrics(p=>({...p,[m]:v}))}/>
              </div>
            ))}
          </>}
          {step===8&&<>
            ...
            {nps!==null&&nps<=6&&(
              <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:14,padding:"18px",marginBottom:14}}>
                ...
                <div style={{display:"flex",gap:10,marginBottom:wantsCB?14:0}}>
                  {["Yes please","No thanks"].map(opt=>(
                    <button key={opt} onClick={()=>setWantsCB(opt==="Yes please")}
                      style={{padding:"12px 18px",borderRadius:22,cursor:"pointer",fontSize:13,fontFamily:"'Space Mono',monospace",WebkitTapHighlightColor:"transparent",
                ...
                {wantsCB&&<input value={contact} onChange={e=>setContact(e.target.value)} placeholder="Your WhatsApp number" style={{...inp,marginTop:12}}/>}
              </div>
            )}
          </>}
          {step===9&&<>
      ...
      {/* FOOTER */}
      <div style={{flexShrink:0,padding:"12px 16px",background:"#000",borderTop:"1px solid #111"}}>
        <button
        ...
        {step===1&&(
          <div style={{textAlign:"center",marginTop:12}}>
            <button onClick={onAdmin}
              style={{background:"none",border:"1px solid #39FF1444",borderRadius:20,padding:"8px 20px",fontFamily:"'Space Mono',monospace",fontSize:11,color:"#39FF14",cursor:"pointer",WebkitTapHighlightColor:"transparent",letterSpacing:1}}>
              🔐 Admin Access
            </button>
          </div>
        )}
...
function AdminLogin({onSuccess}) {
  ...
  return (
    <div style={{height:vh,background:"#000",display:"flex",flexDirection:"column",justifyContent:"center",padding:"32px 20px",overflowY:"auto"}}>
      ...
      <button onClick={login} disabled={!email.trim()||!password.trim()||loading}
        style={{width:"100%",padding:"20px",borderRadius:14,border:"none",background:!email.trim()||!password.trim()||loading?"#1a1a1a":"#39FF14",color:!email.trim()||!password.trim()||loading?"#444":"#000",fontWeight:700,fontSize:18,cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",WebkitTapHighlightColor:"transparent"}}>
              ...
              <tbody>
                {[...data].reverse().map((r,i)=>(
                  <tr key={i} style={{borderBottom:"1px solid #0a0a0a"}}>
                    ...
                    <td style={{padding:"9px 6px",color:"#bbb",maxWidth:90,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.centre}</td>
