...
function Stars({value,onChange,size=64}) {
        ...
        {[1,2,3,4,5].map(s=>{
          ...
          return <button key={s} onClick={()=>onChange?.(s)}
            ...
            style={{background:"none",border:"none",cursor:onChange?"pointer":"default",fontSize:size,lineHeight:1,padding:6,WebkitTapHighlightColor:"transparent",
              ...
              filter:on?"drop-shadow(0 0 10px #39FF14)":"grayscale(1) opacity(.2)"}}>⭐</button>;
        })}
      </div>
      {(h??value)>0&&<div style={{textAlign:"center",color:"#39FF14",fontFamily:"'Space Mono',monospace",fontSize:28,marginTop:16,letterSpacing:1}}>{labels[h??value]}</div>}
    </div>
  );
}
...
function NPSPicker({value,onChange}) {
  ...
  return (
    <div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",marginBottom:14}}>
        {[...Array(11).keys()].map(n=>{
          ...
          return <button key={n} onClick={()=>onChange(n)}
            style={{width:64,height:64,borderRadius:14,border:sel?`3px solid ${col(n)}`:"2px solid #222",
              background:sel?col(n)+"22":"#111",color:sel?col(n):"#555",
              fontFamily:"'Space Mono',monospace",fontSize:22,fontWeight:sel?700:400,
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontFamily:"'Space Mono',monospace",fontSize:20,color:"#333"}}>
...
function FeedbackPage({onAdmin}) {
  ...
  const HEADER_H=130;
  const FOOTER_H=step===1?180:150;

  const lbl={fontFamily:"'Space Mono',monospace",fontSize:28,color:"#444",letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:14};
  const h1={fontSize:52,fontWeight:700,color:"#fff",marginBottom:20,fontFamily:"'Rajdhani',sans-serif",lineHeight:1.2};
  const inp={width:"100%",background:"#0f0f0f",border:"1.5px solid #1e1e1e",borderRadius:16,color:"#fff",fontFamily:"'Space Mono',monospace",fontSize:22,padding:"22px",outline:"none",boxSizing:"border-box",WebkitAppearance:"none"};
  const CTAbtn=(active)=>({
    ...
    boxShadow:active?"0 0 28px #39FF1455":"none",
    WebkitTapHighlightColor:"transparent",
    letterSpacing:0.5,
  });
  const backBtn={width:"100%",padding:"26px",borderRadius:20,border:"1.5px solid #222",background:"transparent",color:"#555",fontWeight:700,fontSize:28,cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",WebkitTapHighlightColor:"transparent",marginTop:12};
  ...
  if(step===10) return (
    <div style={{height:vh,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",textAlign:"center"}}>
      <div style={{fontSize:100,marginBottom:24}}>{rating>=4?"🏆":rating<=2?"🙏":"✅"}</div>
      <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:52,fontWeight:700,color:"#fff",marginBottom:16}}>
        {rating>=4?"You made our day!":rating<=2?"We've flagged this.":"Thanks! We'll improve."}
      </div>
      <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,color:"#555",lineHeight:2,marginBottom:48}}>
        {rating<=2?"Our team will make it right. 💪":"Your feedback helps Hoppr grow. 🎾"}
      </div>
      <button onClick={reset} style={{...CTAbtn(true),width:"auto",padding:"28px 56px",fontSize:28}}>Give More Feedback</button>
    </div>
  );
  ...
  return (
    <div style={{height:vh,background:"#000",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* HEADER */}
      <div style={{height:HEADER_H,padding:"20px 22px 0",flexShrink:0,display:"flex",flexDirection:"column"}}>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:52,fontWeight:700,color:"#39FF14",letterSpacing:3,textShadow:"0 0 18px #39FF1455"}}>HOPPR</div>
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:14,color:"#222",letterSpacing:4}}>SPORTS CENTRES</div>
        </div>
        <div style={{height:5,background:"#111",borderRadius:3,overflow:"hidden",marginBottom:10}}>
          <div style={{height:"100%",width:`${(actualStep/TOTAL)*100}%`,background:"#39FF14",boxShadow:"0 0 10px #39FF14",transition:"width .4s ease",borderRadius:3}}/>
        </div>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:16,color:"#333",letterSpacing:1,textAlign:"center"}}>STEP {actualStep} OF {TOTAL}</div>
      </div>

      {/* BODY — always scrollable */}
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"16px 20px 0"}}>

        {/* STEPS 1-4: option cards */}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {step===1&&CENTRES.map(c=>(
              <button key={c} onClick={()=>setCentre(c)}
                style={{width:"100%",minHeight:90,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 24px",borderRadius:18,
                  border:centre===c?"2.5px solid #39FF14":"1.5px solid #1e1e1e",
                  background:centre===c?"#001800":"#0f0f0f",
                  boxShadow:centre===c?"0 0 20px #39FF1433":"none",
                  cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent",transition:"all .15s"}}>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:36,color:centre===c?"#39FF14":"#ccc",lineHeight:1.3,flex:1}}>🏟️  {c}</span>
                {centre===c&&<span style={{color:"#39FF14",fontSize:32}}>✓</span>}
              </button>
            ))}
            {step===2&&[["🎓","Coaching","For Parents & Kids in coaching"],["🎾","Pay & Play","For walk-in players"]].map(([e,d,sub])=>(
              <button key={d} onClick={()=>setDiv(d)}
                style={{width:"100%",minHeight:160,display:"flex",flexDirection:"column",alignItems:"flex-start",justifyContent:"center",padding:"28px 28px",borderRadius:18,
                  border:division===d?"2.5px solid #39FF14":"1.5px solid #1e1e1e",
                  background:division===d?"#001800":"#0f0f0f",
                  boxShadow:division===d?"0 0 20px #39FF1433":"none",
                  cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent"}}>
                <div style={{fontSize:56,marginBottom:10}}>{e}</div>
                <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:46,color:division===d?"#39FF14":"#fff",marginBottom:6}}>{d}</div>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,color:"#555"}}>{sub}</div>
              </button>
            ))}
            {step===3&&[["👨‍👩‍👧","Parent"],["🏃","Kid / Student"]].map(([e,r])=>(
              <button key={r} onClick={()=>setRole(r)}
                style={{width:"100%",minHeight:140,display:"flex",alignItems:"center",gap:28,padding:"28px 28px",borderRadius:18,
                  border:role===r?"2.5px solid #39FF14":"1.5px solid #1e1e1e",
                  background:role===r?"#001800":"#0f0f0f",
                  boxShadow:role===r?"0 0 20px #39FF1433":"none",
                  cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent"}}>
                <span style={{fontSize:64}}>{e}</span>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:46,color:role===r?"#39FF14":"#ccc"}}>{r}</span>
              </button>
            ))}
            {step===4&&SPORTS.map(s=>(
              <button key={s} onClick={()=>setSport(s)}
                style={{width:"100%",minHeight:100,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 24px",borderRadius:18,
                  border:sport===s?"2.5px solid #39FF14":"1.5px solid #1e1e1e",
                  background:sport===s?"#001800":"#0f0f0f",
                  boxShadow:sport===s?"0 0 20px #39FF1433":"none",
                  cursor:"pointer",textAlign:"left",WebkitTapHighlightColor:"transparent",transition:"all .15s"}}>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:42,color:sport===s?"#39FF14":"#ccc"}}>{s}</span>
                {sport===s&&<span style={{color:"#39FF14",fontSize:32}}>✓</span>}
              </button>
            ))}
          </div>
        </>}
        ...
        {step===5&&<>
          ...
          <div style={{marginTop:48,marginBottom:32}}><Stars value={rating} onChange={setRating} size={72}/></div>
        </>}
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:12,marginTop:8}}>
            {chipOptions.map(c=>(
              <button key={c} onClick={()=>toggle(c)}
                style={{padding:"20px 28px",borderRadius:30,cursor:"pointer",fontSize:26,fontFamily:"'Space Mono',monospace",WebkitTapHighlightColor:"transparent",
                  border:chips.includes(c)?"2px solid #39FF14":"1.5px solid #1e1e1e",
                  background:chips.includes(c)?"#001800":"#0f0f0f",
                  color:chips.includes(c)?"#39FF14":"#777",
                  boxShadow:chips.includes(c)?"0 0 12px #39FF1433":"none"}}>
                {c}
              </button>
            ))}
          </div>
          {chips.includes("Something else")&&(
            <textarea value={otherText} onChange={e=>setOther(e.target.value)} placeholder="Tell us more..." rows={3} style={{...inp,marginTop:18}}/>
          )}
        </>}
        ...
        {step===7&&<>
          ...
          {metricsList.map(m=>(
            <div key={m} style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:18,padding:"24px",marginBottom:16}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:24,color:"#777",marginBottom:18}}>{m}</div>
              <Stars size={48} value={metrics[m]||0} onChange={v=>setMetrics(p=>({...p,[m]:v}))}/>
            </div>
          ))}
        </>}
        ...
        {step===8&&<>
          ...
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,color:"#555",marginBottom:20}}>0 = not at all · 10 = absolutely yes</div>
          <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:18,padding:"24px",marginBottom:18}}>
            <NPSPicker value={nps} onChange={setNps}/>
          </div>
          {nps!==null&&nps<=6&&(
            <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:18,padding:"24px",marginBottom:18}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,color:"#777",marginBottom:18}}>Would you like a callback?</div>
              <div style={{display:"flex",gap:14,marginBottom:wantsCB?18:0}}>
                {["Yes please","No thanks"].map(opt=>(
                  <button key={opt} onClick={()=>setWantsCB(opt==="Yes please")}
                    style={{padding:"18px 26px",borderRadius:30,cursor:"pointer",fontSize:22,fontFamily:"'Space Mono',monospace",WebkitTapHighlightColor:"transparent",
                      border:(opt==="Yes please"?wantsCB:!wantsCB)?"2px solid #39FF14":"1.5px solid #1e1e1e",
              ...
              {wantsCB&&<input value={contact} onChange={e=>setContact(e.target.value)} placeholder="Your WhatsApp number" style={{...inp,marginTop:16}}/>}
            </div>
          )}
        </>}
        ...
        {step===9&&<>
          ...
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,color:"#555",marginBottom:18}}>Optional — we read every word.</div>
          ...
          <div style={{background:"#001500",border:"1px solid #1a3a10",borderRadius:18,padding:"22px",marginTop:18}}>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:18,color:"#39FF14",marginBottom:16,letterSpacing:1}}>YOUR SUMMARY</div>
            {[["Centre",centre],["Division",division],["Sport",sport],["Rating","⭐".repeat(rating)],["NPS",nps!==null?`${nps}/10`:"-"]].map(([k,v])=>(
              <div key={k} style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:20,color:"#444"}}>{k}</span>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:20,color:"#aaa"}}>{v}</span>
              </div>
            ))}
          </div>
        </>}
        ...
        <div style={{height:FOOTER_H+20}}/>
      </div>

      {/* FOOTER — fixed at bottom */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:"16px 20px 28px",background:"linear-gradient(to top,#000 75%,transparent)",zIndex:100}}>
        ...
        {step===1&&(
          <div style={{textAlign:"center",marginTop:16}}>
            <button onClick={onAdmin}
              style={{background:"none",border:"1.5px solid #39FF1455",borderRadius:30,padding:"12px 28px",fontFamily:"'Space Mono',monospace",fontSize:18,color:"#39FF14",cursor:"pointer",WebkitTapHighlightColor:"transparent",letterSpacing:1}}>
              🔐 Admin Access
            </button>
          </div>
        )}
...
function AdminLogin({onSuccess}) {
  ...
  const lbl={fontFamily:"'Space Mono',monospace",fontSize:20,color:"#444",letterSpacing:2,textTransform:"uppercase",display:"block",marginBottom:12};

  return (
    <div style={{minHeight:vh,background:"#000",display:"flex",flexDirection:"column",justifyContent:"center",padding:"36px 24px",overflowY:"auto"}}>
      <div style={{textAlign:"center",marginBottom:48}}>
        <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:52,fontWeight:700,color:"#39FF14",letterSpacing:3,marginBottom:4}}>HOPPR</div>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:14,color:"#222",letterSpacing:4,marginBottom:36}}>SPORTS CENTRES</div>
        <div style={{fontSize:60,marginBottom:18}}>🔐</div>
        <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:42,fontWeight:700,color:"#fff",marginBottom:12}}>Admin Access</div>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:20,color:"#444",lineHeight:1.8}}>Restricted to authorised<br/>Hoppr team only.</div>
      </div>
      ...
      {error&&<div style={{background:"#1a0000",border:"1px solid #7f1d1d",borderRadius:14,padding:"18px 22px",fontFamily:"'Space Mono',monospace",fontSize:20,color:"#f87171",marginBottom:22}}>🚫 {error}</div>}
      <button onClick={login} disabled={!email.trim()||!password.trim()||loading}
        style={{width:"100%",padding:"30px",borderRadius:20,border:"none",
          background:!email.trim()||!password.trim()||loading?"#1a1a1a":"#39FF14",
          color:!email.trim()||!password.trim()||loading?"#444":"#000",
          fontWeight:700,fontSize:32,cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",
          WebkitTapHighlightColor:"transparent"}}>
...
function AdminDashboard({onLogout}) {
  ...
  const tag=(col,txt)=><span style={{fontFamily:"'Space Mono',monospace",fontSize:14,padding:"4px 12px",borderRadius:12,background:col+"22",color:col,border:`1px solid ${col}44`}}>{txt}</span>;
  const pill=(a)=>({padding:"12px 18px",borderRadius:24,border:"none",cursor:"pointer",fontFamily:"'Space Mono',monospace",fontSize:16,background:a?"#39FF14":"#111",color:a?"#000":"#555",whiteSpace:"nowrap",WebkitTapHighlightColor:"transparent"});
  const card={background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:18,padding:"20px",marginBottom:14};
  const sT={fontSize:24,fontWeight:700,color:"#fff",marginBottom:16,marginTop:28,fontFamily:"'Space Mono',monospace"};
  const tabS=(a)=>({flex:1,padding:"18px 4px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"'Space Mono',monospace",fontSize:13,letterSpacing:1,textTransform:"uppercase",color:a?"#39FF14":"#333",borderBottom:a?"2.5px solid #39FF14":"2.5px solid transparent",WebkitTapHighlightColor:"transparent"});

  return (
    <div style={{minHeight:"100vh",background:"#000"}}>
      <div style={{position:"sticky",top:0,background:"#000",borderBottom:"1px solid #111",padding:"20px 20px 0",zIndex:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <div>
            <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:32,fontWeight:700,color:"#39FF14",letterSpacing:2}}>HOPPR</div>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:13,color:"#222",letterSpacing:2}}>ADMIN DASHBOARD</div>
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={load} style={{fontFamily:"'Space Mono',monospace",fontSize:16,padding:"14px 20px",borderRadius:12,border:"1px solid #1a1a1a",background:"#111",color:"#39FF14",cursor:"pointer"}}>
              {loading?"…":"↻ Refresh"}
            </button>
            <button onClick={onLogout} style={{fontFamily:"'Space Mono',monospace",fontSize:16,padding:"14px 20px",borderRadius:12,border:"1px solid #3a1212",background:"#1a0000",color:"#f87171",cursor:"pointer"}}>
              Exit
            </button>
          </div>
        </div>
        <div style={{display:"flex",gap:10,marginBottom:18}}>
          {[["Resp",rows.length,"#fff"],["Avg",`${oAvg}★`,"#39FF14"],["NPS",nps!==null?nps:"—",nps!==null?(nps>=50?"#39FF14":nps>=0?"#facc15":"#f97316"):"#555"],["Esc",`${escRate}%`,"#f87171"]].map(([l,v,c])=>(
            <div key={l} style={{flex:1,background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:14,padding:"14px 8px",textAlign:"center"}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:12,color:"#444",letterSpacing:1,marginBottom:6}}>{l}</div>
      ...
      <div style={{padding:"20px 20px 60px"}}>
        ...
        {esc.length>0&&(
          <div style={{background:"#1a0000",border:"1.5px solid #7f1d1d",borderRadius:16,padding:"16px 20px",marginBottom:20,display:"flex",alignItems:"center",gap:16}}>
            <span style={{fontSize:28}}>🚨</span>
            <div>
              <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#f87171",fontSize:24}}>{esc.length} Active Escalation{esc.length>1?"s":""}</div>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:15,color:"#7f4f4f"}}>Requires immediate attention</div>
            </div>
          </div>
        )}
        ...
        {tab==="overview"&&<>
          ...
          {[["😔 Low (1–2★)",paths.low,"#f87171"],["😐 Mid (3★)",paths.mid,"#facc15"],["😊 High (4–5★)",paths.high,"#39FF14"]].map(([l,v,c])=>(
            <div key={l} style={{...card,display:"flex",alignItems:"center",gap:14}}>
              <span style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#bbb",flex:1}}>{l}</span>
              <div style={{width:100,height:8,background:"#111",borderRadius:4,overflow:"hidden"}}>
                <div style={{width:`${rows.length?v/rows.length*100:0}%`,height:"100%",background:c,borderRadius:4}}/>
              </div>
              <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:c,minWidth:32,textAlign:"right",fontSize:24}}>{v}</span>
            </div>
          ))}
          <div style={sT}>Centre Performance</div>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Space Mono',monospace",fontSize:15,minWidth:360}}>
              <thead><tr style={{borderBottom:"1px solid #1a1a1a"}}>
                {["Centre","Resp","Avg","NPS","Esc"].map(h=><th key={h} style={{padding:"14px 10px",color:"#39FF14",textAlign:"left",letterSpacing:1,whiteSpace:"nowrap"}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {cPerf.map(c=>(
                  <tr key={c.centre} style={{borderBottom:"1px solid #0a0a0a"}}>
                    <td style={{padding:"14px 10px",color:"#bbb",whiteSpace:"nowrap"}}>{c.centre}</td>
                    <td style={{padding:"14px 10px",color:"#fff"}}>{c.count}</td>
                    <td style={{padding:"14px 10px",color:"#39FF14"}}>{c.avg}★</td>
                    <td style={{padding:"14px 10px",color:c.nps>=50?"#39FF14":c.nps>=0?"#facc15":"#f97316"}}>{c.nps??"-"}</td>
                    <td style={{padding:"14px 10px",color:c.esc>0?"#f87171":"#555"}}>{c.esc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {topIssues.length===0&&<div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#444"}}>No issues reported ✅</div>}
          {topIssues.map(([issue,count])=>(
            <div key={issue} style={{...card,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#bbb"}}>{issue}</span>
              <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#f87171",fontSize:26}}>{count}×</span>
            </div>
          ))}
        </>}

        {tab==="escalations"&&<>
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#f87171",marginBottom:18}}>{esc.length} requiring action</div>
          {esc.length===0&&<div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#39FF14"}}>No escalations ✅</div>}
          {esc.map((r,i)=>(
            <div key={i} style={{...card,background:"#0d0000",border:"1px solid #7f1d1d",marginBottom:18}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div style={{display:"flex",gap:8}}>{tag("#f87171","🚨 FLAGGED")}{tag("#f97316",r.division)}</div>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:13,color:"#444"}}>{new Date(r.timestamp).toLocaleString("en-IN",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</span>
              </div>
              <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#fff",fontSize:26,marginBottom:10}}>{r.centre}</div>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#f87171",marginBottom:10}}>{"⭐".repeat(+r.overall_rating)} · {r.issue||"Not specified"}</div>
              {r.issue_detail&&<div style={{fontFamily:"'Space Mono',monospace",fontSize:16,color:"#666",marginBottom:12}}>"{r.issue_detail}"</div>}
              {r.contact&&<div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#39FF14"}}>📱 {r.contact}</div>}
              <div style={{marginTop:16}}>{tag("#facc15","NEW")}</div>
            </div>
          ))}
        </>}
        ...
        {tab==="breakdown"&&<>
          ...
          {mAvg(COACHING_METRICS,"Coaching").map(m=>(
            <div key={m.metric} style={card}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#888"}}>{m.metric}</span>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#39FF14",fontSize:24}}>{m.avg}★</span>
              </div>
          ...
          {sportStats.map(s=>(
            <div key={s.sport} style={{...card,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#fff",fontSize:24}}>{s.sport}</span>
              <div style={{display:"flex",gap:16,alignItems:"center"}}>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:16,color:"#555"}}>{s.count}x</span>
                <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#39FF14",fontSize:24}}>{s.avg}★</span>
              </div>
            </div>
          ))}
        </>}
        ...
        {tab==="comments"&&<>
          <input value={search} onChange={e=>setSrch(e.target.value)} placeholder="Search comments..."
            style={{width:"100%",background:"#0f0f0f",border:"1.5px solid #1e1e1e",borderRadius:16,color:"#fff",fontFamily:"'Space Mono',monospace",fontSize:18,padding:"18px",outline:"none",boxSizing:"border-box",WebkitAppearance:"none",marginBottom:20}}/>
          {comments.length===0&&<div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#444"}}>No comments found.</div>}
          {comments.map((r,i)=>(
            <div key={i} style={card}>
              ...
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:17,color:"#ccc",lineHeight:1.8,marginBottom:14}}>"{r.comment}"</div>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:16,color:"#444"}}>{"⭐".repeat(+r.overall_rating)} · NPS {r.nps} · {new Date(r.timestamp).toLocaleDateString("en-IN")}</div>
            </div>
          ))}
        </>}
        ...
        {tab==="feed"&&<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:"#fff",fontSize:26}}>Live Feed</div>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:15,color:"#444"}}>{new Date().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</div>
          </div>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'Space Mono',monospace",fontSize:14,minWidth:400}}>
              <thead><tr style={{borderBottom:"1px solid #1a1a1a"}}>
                {["Time","Centre","Div","★","Path","🚨"].map(h=><th key={h} style={{padding:"14px 8px",color:"#39FF14",textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {[...data].reverse().map((r,i)=>(
                  <tr key={i} style={{borderBottom:"1px solid #0a0a0a"}}>
                    <td style={{padding:"13px 8px",color:"#555",whiteSpace:"nowrap"}}>{new Date(r.timestamp).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</td>
                    <td style={{padding:"13px 8px",color:"#bbb",maxWidth:110,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.centre}</td>
                    <td style={{padding:"13px 8px",color:"#888"}}>{r.division==="Coaching"?"🎓":"🎾"}</td>
                    <td style={{padding:"13px 8px",color:"#39FF14"}}>{r.overall_rating}★</td>
                    <td style={{padding:"13px 8px"}}><span style={{color:r.path==="low"?"#f87171":r.path==="mid"?"#facc15":"#39FF14",fontSize:13,textTransform:"uppercase"}}>{r.path}</span></td>
                    <td style={{padding:"13px 8px"}}><span style={{color:r.escalated==="YES"?"#f87171":"#333"}}>{r.escalated==="YES"?"🚨":"—"}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
